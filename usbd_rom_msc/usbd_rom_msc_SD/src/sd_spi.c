...
static SD_RESULT sd_send_command(uint16_t cmd, uint32_t data) {
...
SD_RESULT init_sd_spi(SD_CardInfo *cardinfo) {
  ...
  setupSpiMaster(SystemCoreClock / 12000000 - 1);//12Mhz
  
  // Read and decode CID register
  ret = sd_read_cid(&cardinfo->SD_cid, cardinfo->CardType);
  if (ret != SD_OK) {
    return ret;
  }
  
  // Read and decode CSD register
  ret = sd_read_csd(&cardinfo->SD_csd, cardinfo->CardType);
  if (ret != SD_OK) {
    return ret;
  }
...
SD_RESULT sd_read_cid(SD_CID *sd_cid,CARD_TYPE ct) {
...
SD_RESULT sd_read_csd(SD_CSD *sd_csd, CARD_TYPE ct) {
