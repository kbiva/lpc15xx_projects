...
int main(void) {
  ...
  putline_UART("Starting\r\n");

  sd_ret = init_sd_spi(&cardinfo);
  ...
  sprintf(buffer,"\r\nCapacity:%llu(bytes)",cardinfo.CardCapacity);
  putline_UART(buffer);
  sprintf(buffer,"\r\n%llu(Kbytes)",cardinfo.CardCapacity/1024);
  putline_UART(buffer);
  sprintf(buffer,"\r\n%llu(Mbytes)",cardinfo.CardCapacity/(1024*1024));
  putline_UART(buffer);
  sprintf(buffer,"\r\n%.2f(Gbytes)",(((float)cardinfo.CardCapacity/(1024*1024))/1024));
  putline_UART(buffer);
  
  if(cardinfo.SD_csd.CSDStruct==0) {
    sprintf(buffer,"\r\nDeviceSize:%d(%dMb)",cardinfo.SD_csd.DeviceSize,(cardinfo.SD_csd.DeviceSize+1)*(1<<(cardinfo.SD_csd.DeviceSizeMul+2))*(1<<cardinfo.SD_csd.RdBlockLen)/(1024*1024));
  } else if(cardinfo.SD_csd.CSDStruct==1) {
    sprintf(buffer,"\r\nDeviceSize:%d(%dMb)",cardinfo.SD_csd.DeviceSize,(cardinfo.SD_csd.DeviceSize+1)*512/1024);
  } else {
    sprintf(buffer,"\r\nDeviceSize:(unknown)");
  }
  putline_UART(buffer);
